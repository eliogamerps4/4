...
function setupRW() {
	...
	if(localStorage.autoExploit=="true")
		debug_log(" Webkit Completado activar Exploit para piratear PS4");
	else
		debug_log(" Webkit Completado activar el  Exploit para piratear PS4");
...
function toggle_payload(pld){
	if(pld == "exploit"){
		...
		document.getElementById("progress").innerHTML="Esperando payload.. enviar por el pueto 9021..";
...
function payload_finished(payload)
{
	if(payload == "cargando paquete"){
		setTimeout(function(){document.getElementById("progress").innerHTML="Esperando payload...envia por el puerto 9021"; }, 7000);
	} else if(payload != "exploit" && payload != "exploit_old"){
		setTimeout(function(){document.getElementById("progress").innerHTML="Pirateo PS4 7.55 Completado por (Elio)"; }, 7000);
	}
}
...
function sprayHTMLTextArea() {
	debug_log( "Iniciando Webkit xploit Por HTML...");
